"""Faster production runner: history-aware selection, metrics only on top candidates."""
...
def run_weekly_brief(days=21,per_query=80,minimum_score=55,hard_max=60,output_dir='Output/weekly',history_path='data/weekly_history.json'):
 ...
 metric_pool=[r for r in ranked if int(r.get('relevance_score',0))>=40][:20]
 metric_pool=annotate_journal_metrics(metric_pool); metric_pool=fill_missing(metric_pool)
