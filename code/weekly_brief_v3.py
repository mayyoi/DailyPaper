...
This runner reuses v2 rendering/annotation logic but selects papers dynamically
from the metric-complete pool, so a shortage of JCR metadata does not silently
shrink an otherwise strong week to three papers.
"""
...
def run_weekly_brief(days=21, per_query=80, minimum_score=55, hard_max=60, output_dir="Output/weekly"):
    ...
    # Build a sufficiently broad quality pool first. JCR completeness is a hard gate,
    # but it is evaluated before the final count so missing metrics do not dominate selection.
    ...
    complete = [r for r in quality_pool if (r.get("journal_metrics") or {}).get("jif") is not None and (r.get("journal_metrics") or {}).get("jcr_quartile")]
    strong = [r for r in complete if int(r.get("relevance_score",0)) >= minimum_score]
    # If fewer than five strong papers survive all completeness gates, expand to the
    # score>=40 pool. This is intentionally dynamic and can still return fewer than five.
    selected = strong if len(strong) >= 5 else [r for r in complete if int(r.get("relevance_score",0)) >= 40]
    if hard_max > 0:
        selected = selected[:hard_max]
    if not selected:
        raise RuntimeError("No paper has both adequate DR×lipid relevance and a complete current JCR-based JIF + quartile record; refusing to send an incomplete brief.")

    # Final metadata pass: author affiliations and corresponding author where publicly identified.
