...
presented as independently verified Clarivate JCR data. This is intentional:
search engines can surface stale or conflicting third-party values.
"""
...
HEADERS = {
    ...
    "Chrome/140 Safari/537.36 DailyPaper/1.0"
}
...
def _openalex_source(journal: str, issn: str = "") -> Dict:
    try:
        ...
        data = requests.get(
            f"{OPENALEX}/sources", params=params, timeout=20, headers=HEADERS
        ).json()
...
def _homepage_impact_factor(journal: str, issn: str = "") -> Dict:
    ...
    try:
        ...
        r.raise_for_status()
        text = _clean(
            re.sub(r"<script.*?</script>|<style.*?</style>|<[^>]+>", " ", r.text, flags=re.I | re.S)
        )
        patterns = [
            ...
            if m:
                ...
                if 0 <= value <= 100:
                    return {
                        ...
                        "jcr_quartile": "",
                        "jcr_category_rank": "",
                        "metric_source": "Journal homepage (explicit Impact Factor label; not independently verified as Clarivate JCR)",
                        "metric_url": homepage,
                        "journal": journal,
                        "issn": issn,
                    }
...
def _bing_results(query: str, timeout: int = 20) -> List[Dict[str, str]]:
    """Return Bing result title/snippet/url records without requiring an API key."""
    try:
        r = requests.get(BING, params={"q": query, "count": 10, "setlang": "en-US"}, timeout=timeout, headers=HEADERS)
        r.raise_for_status()
        html = r.text
        results = []
        for block in re.findall(r'<li class="b_algo".*?</li>', html, flags=re.I | re.S):
            ...
            if title_m or snippet_m:
                results.append({
                    "title": _clean(re.sub(r"<[^>]+>", " ", title_m.group(1))) if title_m else "",
                    "snippet": _clean(re.sub(r"<[^>]+>", " ", snippet_m.group(1))) if snippet_m else "",
                    "url": html_lib.unescape(url_m.group(1)) if url_m else "",
                })
        return results
    except Exception as exc:
        print(f"[bing] search failed for {query!r}: {exc}")
        return []


def _extract_jif(text: str) -> float | None:
    patterns = [
        r"(?:impact\s+factor|journal\s+impact\s+factor|JIF)\s*(?:for\s+2025|2025)?\s*[:=]?\s*(\d+(?:\.\d+)?)",
        r"(?:2025\s+)?impact\s+factor\s*[:=]?\s*(\d+(?:\.\d+)?)",
    ]
    for pat in patterns:
        m = re.search(pat, text, flags=re.I)
        if m:
...
def _extract_quartile(text: str) -> str:
    # Require an explicit quartile cue; do not interpret a bare Q1/Q2 in an unrelated context.
    patterns = [
        r"(?:JCR\s+)?quartile\s*[:=\-]?\s*(Q[1-4])",
        r"(?:JCR|WOS|Web\s+of\s+Science)[^Q]{0,50}(Q[1-4])",
        r"\b(Q[1-4])\s+(?:journal|quartile)",
    ]
    for pat in patterns:
        m = re.search(pat, text, flags=re.I)
        if m:
...
def _bing_journal_metrics(journal: str, issn: str = "") -> Dict:
    """Use Bing only after structured sources fail.

    We look for evidence where the same search result explicitly mentions the
    journal and an Impact Factor and/or JCR quartile. The output records Bing
    provenance so it cannot be mistaken for an official Clarivate lookup.
    """
    queries = [
        f'"{journal}" "impact factor" "JCR quartile"',
        f'"{journal}" "impact factor" Q1 Q2 Q3 Q4',
    ]
    if issn:
        queries.insert(0, f'"{journal}" "{issn}" "impact factor" "JCR"')
    candidates = []
    ...
    for item in candidates:
        ...
        if "impact factor" not in combined.lower() and "jif" not in combined.lower():
            continue
        # Prefer results that contain the journal title tokens or ISSN.
        title_norm = _norm(item.get("title", ""))
        snippet_norm = _norm(item.get("snippet", ""))
        journal_hit = target and (target in title_norm or target in snippet_norm)
        issn_hit = bool(issn and issn in combined)
        if not (journal_hit or issn_hit):
            continue
        jif = _extract_jif(combined)
        quartile = _extract_quartile(combined)
        if jif is None and not quartile:
            continue
        score = (4 if journal_hit else 2) + (2 if jif is not None else 0) + (2 if quartile else 0)
        # Prefer a result that supplies both fields in the same snippet/title.
    ...
    return {
        ...
        "jcr_release": 2026 if quartile else None,
        "jcr_quartile": quartile,
        "jcr_category_rank": "",
        "metric_source": "Bing web search (third-party evidence; not independently verified as Clarivate JCR)",
        "metric_url": item.get("url", BING + "?q=" + quote_plus(queries[0])),
...
def lookup_journal_metric(journal: str, issn: str, metrics: Dict[str, Dict]) -> Dict:
    ...
    return {
        ...
        "jcr_quartile": "未检索到",
        "jcr_category_rank": "",
        "metric_source": "未能从公开JCR-based目录、期刊首页或Bing搜索可靠匹配；不填猜测值",
        "metric_url": BASE + "/",
    }
