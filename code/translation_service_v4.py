...
def google_json(url,params,headers):
    r=requests.get(url,params=params,headers=headers,timeout=12);r.raise_for_status();d=r.json()
    if isinstance(d,list) and d and isinstance(d[0],list):return ''.join(p[0] for p in d[0] if isinstance(p,list) and p and p[0])
    if isinstance(d,dict) and d.get('sentences'):return ''.join(s.get('trans','') for s in d['sentences'])
    return ''
def google_translate(c):
    ...
    endpoints=[('google-web','https://translate.google.com/translate_a/single',{'client':'gtx','sl':'en','tl':'zh-CN','dt':'t','q':c}),('google-chrome','https://clients5.google.com/translate_a/single',{'client':'dict-chrome-ex','dj':'1','dt':'t','sl':'en','tl':'zh-CN','q':c}),('google-api','https://translate.googleapis.com/translate_a/single',{'client':'gtx','sl':'en','tl':'zh-CN','dt':'t','q':c})]
    ...
    try:
        url='https://translate.google.com/m?sl=en&tl=zh-CN&q='+quote_plus(c);r=requests.get(url,headers=headers,timeout=12);r.raise_for_status();m=re.search(r'<div[^>]*class=["\']result-container["\'][^>]*>(.*?)</div>',r.text,re.S|re.I)
        if m:
...
def openai_translate(c):
    ...
    if not key:return '',''
    model=os.getenv('TRANSLATION_OPENAI_MODEL','gpt-5').strip();headers={'Authorization':f'Bearer {key}','Content-Type':'application/json'}
    prompt=('Translate the following biomedical abstract from English to professional Simplified Chinese. Preserve gene/protein names, acronyms, drug names, units, numbers, statistical values, and abbreviations exactly when appropriate. Do not summarize, omit, or add claims. Return only the Chinese translation.\n\n'+c)
    try:
        r=requests.post('https://api.openai.com/v1/responses',headers=headers,json={'model':model,'input':prompt,'temperature':0},timeout=30);r.raise_for_status();d=r.json();val=d.get('output_text','')
        if not val:
...
def translate(text):
    text=clean(text)
    if not text:return ''
    providers=['openai','google'] if os.getenv('OPENAI_API_KEY','').strip() else ['google'];out=[]
    for c in chunks(text):
        val=''
        for p in providers:
            _,val=openai_translate(c) if p=='openai' else google_translate(c)
            if val:break
        if not val:return ''
        out.append(val)
    x=apply_glossary(clean(' '.join(out)));return x if quality(text,x) else ''
def zh(text):
    o=clean(text)
    if not o:return ''
    if o.lower() in {'no abstract available.','no abstract available','abstract not available','not available'}:return '暂无摘要（原文未提供摘要）'
    t=translate(o)
    if t:return t
    g=apply_glossary(o)
